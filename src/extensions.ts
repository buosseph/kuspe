...
export type ExtensionManager<Ext extends Extension = Extension> =
	PluginManager<Ext>
	& { order: ()=> Ext[] };
 ...
 */
const manager = <Ext extends Extension = Extension>(): ExtensionManager<Ext> => {
	const manager = plugins<Ext>();
	const order= (): Ext[] => {
		const extensions = manager.registry;
		...
		for (const ext of orderedDependencies) {
			...
			const feature = ext[0];
			const found = manager.registry.find(ext => ext.provides[0] === feature);
